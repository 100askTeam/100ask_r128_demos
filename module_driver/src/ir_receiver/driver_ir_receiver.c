...
// 定义引脚
#define IR_RECEIVER_PIN			GPIOA(10)

/* 环形缓冲区: 用来保存解析出来的按键,可以防止丢失 */
#define BUF_LEN 				CONFIG_100ASK_MODULE_DRIVER_IR_RECEIVER_BUFFER
...
static hal_irqreturn_t IRReceiver_IRQ_Callback(void *data)
{
#if 1
    uint64_t now_time;
     ...
     */
    if (now_time - pre_time > 100000000) 
    {
        g_IRReceiverIRQ_Cnt = 0;
    }
    pre_time = now_time;
    
	g_IRReceiverIRQ_Timers[g_IRReceiverIRQ_Cnt] = now_time;
...
void IRReceiver_Init(void)
{
	...
	hal_gpio_set_pull(IR_RECEIVER_PIN, GPIO_PULL_UP);
    hal_gpio_set_direction(IR_RECEIVER_PIN, GPIO_DIRECTION_INPUT);
    hal_gpio_pinmux_set_function(IR_RECEIVER_PIN, GPIO_MUXSEL_IN);

	// 存放 CLK，DT 中断号
    uint32_t irq_ir_receiver_pin;

    // 申请 IR_RECEIVER_PIN 为中断引脚，rising 触发
    ret = hal_gpio_to_irq(IR_RECEIVER_PIN, &irq_ir_receiver_pin);
    if (ret < 0){
        printf("gpio to irq error, irq num:%d error num: %d\n", irq_ir_receiver_pin, ret);
    }
    // 绑定中断处理函数
    ret = hal_gpio_irq_request(irq_ir_receiver_pin, IRReceiver_IRQ_Callback, IRQ_TYPE_EDGE_BOTH, NULL);
    if (ret < 0){
        printf("request irq error, irq num:%d error num: %d\n", irq_ir_receiver_pin, ret);
    }
    // 启用中断
    ret = hal_gpio_irq_enable(irq_ir_receiver_pin);
    if (ret < 0){
 ...
 * 2023/08/04        V1.0     韦东山       创建
 ***********************************************************************/
int IRReceiver_Test(int argc, char *argv[])
{
    uint8_t dev, data;
    int len;
	...
	while (1)
    {
		if (!IRReceiver_Read(&dev, &data))
		{
			printf("IR Receiver: 0x%x, \
					Device  Data: 0x%x, \
					Key name: %s\n", dev, data, IRReceiver_CodeToString(data));
		}
		else printf("No Data\n");
...
FINSH_FUNCTION_EXPORT_CMD(IRReceiver_Test, test_ir_receiver, module driver 100ask test IR receiver);
