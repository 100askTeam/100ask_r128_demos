...
//#define g_iic_addr 0x78

//#define FRAME_WIDTH		128
//#define FRAME_HEIGHT		64
#define FRAME_WIDTH			CONFIG_USE_100ASK_MODULE_DRIVER_IIC_OLED_WIDTH
#define FRAME_HEIGHT		CONFIG_USE_100ASK_MODULE_DRIVER_IIC_OLED_HEIGHT
#define FRAME_BUFFER_SIZE	((FRAME_WIDTH * FRAME_HEIGHT) / 8)


static twi_port_t g_iic_port = 1;
static uint16_t   g_iic_addr = 0x3c;

static uint8_t oled_drv_buf[FRAME_BUFFER_SIZE];
 ...
 *  返回值：0-成功, 其他值失败
 */
static twi_status_t OLED_WriteCmd(uint8_t cmd)
{
    twi_port_t port = g_iic_port;
    uint8_t tmpbuf[2];
    twi_msg_t msg;
 ...
 *  返回值：0-成功, 其他值失败
 */
static twi_status_t OLED_WriteData(uint8_t data)
{
    twi_port_t port = g_iic_port;
    uint8_t tmpbuf[2];
    twi_msg_t msg;
...
static int OLED_WriteNBytes(uint8_t *buf, uint16_t length)
{
    ...
    uint8_t * tmpbuf = (uint8_t *)malloc((length + 5) * sizeof(uint8_t));

    tmpbuf[0] = 0x40;
    write_size += 1;

    memcpy(tmpbuf + write_size, buf, length);
    write_size += length;
...
void OLED_PutChar(uint8_t x, uint8_t y, char c)
{
    ...
    OLED_WriteNBytes((uint8_t*)&ascii_100ask_font[c][0], 8);
    
    OLED_SetPosition(page + 1, col);
    OLED_WriteNBytes((uint8_t*)&ascii_100ask_font[c][8], 8);
}
...
void OLED_Flush(void)
{
      ...
      for(i = 0; i < 8; i++)
      {
         ...
         //OLED_WriteCmd(0x40);
         //for(n=0;n<128;n++)
         //  OLED_WriteData(oled_drv_buf[128*i+n]);
         OLED_WriteNBytes(&oled_drv_buf[128*i], 128);
    }
}
...
{
    memset(oled_drv_buf, 0, sizeof(oled_drv_buf));
}
 ...
 *  返回值：无
 */
void OLED_Init(twi_port_t port, uint16_t iic_addr)
{   
    g_iic_port = port;
    g_iic_addr = iic_addr;
...
void OLED_Test(void)
{
	...
	while (1)
	{
        ...
        OLED_PrintString(0, 4, "Hello 100ask");
        OLED_PrintString(0, 6, "https://www.100ask.net");
	}
}