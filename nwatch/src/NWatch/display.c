...
static void crt_animation()
{
	...
	LOOP(rows, i)     
	{
		memset(&g_framebuffer[i * FRAME_WIDTH], 0, FRAME_WIDTH);
		memset(&g_framebuffer[FRAME_BUFFER_SIZE - FRAME_WIDTH - (i * FRAME_WIDTH)], 0, FRAME_WIDTH);
	}
	...
	if(prows) // Partial rows & edge line
	{
		...
		LOOP(FRAME_WIDTH, i)
		{
			g_framebuffer[idxStart] = (g_framebuffer[idxStart] & a) | c;
			idxStart++;

			g_framebuffer[idxEnd] = (g_framebuffer[idxEnd] & b) | d;
			idxEnd--;
		}
	}
	else if(height) // Edge line
	{
		...
		memset(&g_framebuffer[pos], 0x01, lineWidth);

		if(height != FRAME_HEIGHT / 2)
		{
			pos = (height / 8) * FRAME_WIDTH;
			LOOPR(FRAME_WIDTH, x)
				g_framebuffer[pos + x] |= 0x01;
		}		
	}
