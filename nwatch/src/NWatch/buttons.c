...
typedef enum {
  CONTROL_DEVICE_ROTARY_ENCODER = 1,
  CONTROL_DEVICE_IR,
  CONTROL_DEVICE_ROTARY_ENCODER_AND_IR,
} e_control_device;

static s_button buttons[BTN_COUNT];
static uint16_t g_control_device = CONTROL_DEVICE_ROTARY_ENCODER;

static void processButtons(uint16_t control_device);
static void processButton(s_button*, BOOL);
static byte bitCount(byte);

//EMPTY_INTERRUPT(PCINT0_vect);

void buttons_init(uint16_t control_device)
{
  buttons_startup(control_device);
...
void buttons_update(void)
{
  ...
  {
    lastUpdate = now;
    processButtons(g_control_device);
  }
}
...
void buttons_startup(uint16_t control_device)
{
  g_control_device = control_device;

  switch (g_control_device)
  {
    case CONTROL_DEVICE_ROTARY_ENCODER:
      /* code */
      RotaryEncoder_Init();
      break;
    case CONTROL_DEVICE_IR:
      IRReceiver_Init();
      break;
    case CONTROL_DEVICE_ROTARY_ENCODER_AND_IR:
      RotaryEncoder_Init();
      IRReceiver_Init();
      break;
...
static void processButtons(uint16_t control_device)
{
  ...
  if(control_device == CONTROL_DEVICE_ROTARY_ENCODER)
  {
  ...
  else if(control_device == CONTROL_DEVICE_IR)
	{
  ...
  else if(control_device == CONTROL_DEVICE_ROTARY_ENCODER_AND_IR)  // IR priority
  {
    int32_t count, speed, key;
    static uint8_t dev, data, last_data;
